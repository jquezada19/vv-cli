...
| errors | grep-stable: `kind: message — next: <command>` on one line (plus a `did you mean:` line after a name miss); control characters in a token are escaped, and the `next` step never comes from a token |
