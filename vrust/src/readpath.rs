...
pub enum Outcome { Done(i32), Fallback }
...
pub fn fm_bounds(lines: &[&str]) -> usize {
    ...
    for i in 1..lines.len() {
        if lines[i].trim_end_matches('\r') == "---" { return i + 1; }
    }
    0
}
...
fn fence_line(l: &str) -> Option<(char, usize, &str)> {
    ...
    let mut idx = 0;
    let b = l.as_bytes();
    while idx < 3 && idx < b.len() && b[idx] == b' ' { idx += 1; }
    let rest = &l[idx..];
    ...
    for i in start..lines.len() {
        ...
        match marker {
            ...
            Some((mc, mn)) => {
                masked[i] = true;
                if let Some((ch, n, info)) = m {
                    if ch == mc && n >= mn && info.trim().is_empty() { marker = None; }
                }
            }
        }
    }
...
pub fn parse<'a>(text: &'a str) -> (Vec<&'a str>, Vec<Sec>) {
    ...
    for i in 0..fm_end { fenced[i] = true; }
    ...
    let mut secs = vec![Sec { id: "H0".into(), level: 0, title: "(preamble)".into(),
                              start: 0, end: first }];
    for (j, (i, lvl, title)) in heads.iter().enumerate() {
        let end = heads.get(j + 1).map_or(lines.len(), |h| h.0);
        secs.push(Sec { id: format!("H{}", j + 1), level: *lvl, title: title.clone(),
                        start: *i, end });
    }
    (lines, secs)
}
...
pub fn find_sec<'a>(secs: &'a [Sec], sid: &str) -> Option<&'a Sec> {
    ...
    if wl == "(preamble)" || wl == "preamble" {
        return secs.iter().find(|s| s.title == "(preamble)" || s.id == "H0");
    }
    let matches: Vec<&Sec> = secs.iter()
        .filter(|s| s.title.trim().to_lowercase() == wl).collect();
...
pub fn resolve(vault: &Path, ref_: &str) -> Option<PathBuf> {
    ...
    let hits: Vec<&PathBuf> = files.iter().filter(|p| {
        p.file_name().and_then(|n| n.to_str())
            .map(|n| n.strip_suffix(".md").unwrap_or(n).to_lowercase() == want)
            .unwrap_or(false)
    }).collect();
    if hits.len() == 1 { Some(hits[0].clone()) } else { None }   // 0 or 2+: python
}
...
pub fn log_metrics(op: &str, t0: std::time::Instant, out_bytes: usize, cf: u64) {
    ...
    let now = std::process::Command::new("date").arg("+%Y-%m-%dT%H:%M:%S").output();
...
pub fn run(cmd: &str, args: &[String], vault: &Path) -> Outcome {
    ...
    match cmd {
        "resolve" if args.len() == 1 => {
            let fp = match resolve(vault, &args[0]) { Some(f) => f, None => return Outcome::Fallback };
        ...
        "outline" if args.len() == 1 => {
            ...
            for s in &secs {
                ...
                let hashes = if s.level > 0 { "#".repeat(s.level) } else { "-".into() };
                buf.push_str(&format!("{}\t{}\t{}\t{}B\t{}\n",
                    s.id, hashes, s.title, t.chars().count(), sha8(&t)));  // python len() = CHARS
            }
            let n = emit(&buf);
        ...
        "read" if args.len() == 2 => {
            ...
            let text = match String::from_utf8(bytes) { Ok(t) => t, Err(_) => return Outcome::Fallback };
            let (lines, secs) = parse(&text);
            let s = match find_sec(&secs, &args[1]) { Some(s) => s, None => return Outcome::Fallback };
            let t = sec_text(&lines, s);
            let n = emit(&format!("{}\n--sha8:{}\n", t, sha8(&t)));
...
pub fn split_fm(t: &str) -> Option<&str> {
    ...
    while i < b.len() {
        if b[i] == b'\n' {
            let mut fm_end = i;
            if fm_end > 0 && b[fm_end-1] == b'\r' { fm_end -= 1; }
            let after = &rest[i+1..];
